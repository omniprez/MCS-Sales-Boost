...
const GrossProfitCard = () => {
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/dashboard/gp-summary'],
    queryFn: async () => {
      const response = await fetch('/api/dashboard/gp-summary', {
        credentials: 'include'
      });
      if (!response.ok) {
        throw new Error('Failed to fetch gross profit summary');
      }
  ...
  if (error) {
    return (
      <Card className="bg-white p-6 metric-card">
        <div className="text-sm font-medium text-gray-500 mb-2">Gross Profit</div>
  ...
  return (
    <Card className="bg-white p-4 h-full shadow-md border border-gray-100 hover:shadow-lg transition-shadow">
      <div className="flex justify-between items-start mb-2">
        <h3 className="text-sm font-medium text-blue-600">Gross Profit</h3>
      ...
      <div className="text-xs text-gray-500 mb-2">(40% of Revenue)</div>

      <div className="h-2 w-full bg-gray-200 rounded-full mb-2">
        <div
          className={`h-2 rounded-full ${isPositiveTrend ? 'bg-green-500' : 'bg-red-500'}`}
